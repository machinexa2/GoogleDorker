...
class GoogleDork:
    ...
    def search(self, query):
        ...
        print("{} Trying this payload after sleeping 7-15s: {}".format(self.color.information, colored(URL.split('?')[-1][2:], color='cyan')))
        sleep(ri(7,15))
        if response.status_code == 200:
            ...
            for go_get in soup.find_all('div', class_='r'):
                ...
                if anchors:
                    ...
                    print("{} Obtained {} from dork {}".format(self.color.good, colored(title, color='cyan'), colored(query, color='cyan')))
                    returner.append("Title:{}, Link:{}, Dork:{}\n".format(title, link, query))
        else:
            print("{} Sleeping due to error for 30-40s".format(self.color.bad))
            sleep(ri(30,40))
        return returner
